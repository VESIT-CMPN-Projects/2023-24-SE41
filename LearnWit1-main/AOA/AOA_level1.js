...
const questionsPromise = fetch("http://localhost:5000/api/data")
  .then(response => {
    if (!response.ok) {
  ...
  .then(data => {
    questions = data; // Populate the questions array
    selectedOptions = new Array(questions.length).fill(null); // Initialize selectedOptions array
...
// Wait for questionsPromise to resolve before executing the code that depends on questions
